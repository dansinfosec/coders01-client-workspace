...
const redenen = [
  `Een onafhankelijke vakgarage met vaste klanten${company.foundedYear !== null ? ` sinds ${company.foundedYear}` : ""}`,
...
export function VacaturePage() {
  const subject = encodeURIComponent(`Open sollicitatie — ${company.name}`);

  return (
    <>
      <SEO
        title={`Vacatures & open sollicitatie — werken bij ${company.name}`}
        description={`Werken bij ${company.name} in ${company.address.city}? Wij staan altijd open voor gemotiveerde monteurs en collega's. Stuur een open sollicitatie.`}
        path="/vacatures"
      ...
      <PageHero
        eyebrow="Vacatures"
        title={`Werken bij ${company.name}`}
        intro={`Versterk ons team in ${company.address.city}. Ook zonder actuele vacature ontvangen we graag uw open sollicitatie.`}
          ...
          <div className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:gap-16">
            <div className="max-w-prose">
              ...
              <p className="mt-4 leading-relaxed text-text-body">
                Bij {company.name} werken gediplomeerde monteurs met hart voor het vak. Groeit ons werk, dan groeit ons
              ...
              <h3 className="mt-8 font-display text-lg font-bold text-text-strong">Waarom {company.name}</h3>
