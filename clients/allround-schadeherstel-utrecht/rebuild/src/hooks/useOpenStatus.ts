...
export interface OpenStatus {
  todayIndex: number;
  isOpen: boolean;
  /** "HH:MM" close time for today, if open now. */
...
/**
 * Compute today's open/closed status from business.hours (visitor's local clock,
 * assumed NL / Europe-Amsterdam for this workshop).
 */
    ...
    for (let i = 1; i <= 7; i++) {
      ...
      if (next?.range) {
        ...
        return {
          ...
          isOpen: false,
          closesAt: null,
          opensLabel: `Opent ${label} om ${fmt(next.range[0])}`,
        };
      }
    }
