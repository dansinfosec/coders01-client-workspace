 ...
 */
import { type Occasion, type Transmissie } from "@/data/occasions";
import { enrichedOccasions as occasions } from "@/lib/enrichOccasions";
...
export function getOccasionFacets(): OccasionFacets {
  ...
  const transmissies = Array.from(
    new Set(occasions.map((o) => o.transmissie).filter((t): t is Transmissie => t !== null)),
  );
